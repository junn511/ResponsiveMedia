...
class JinsSocket(threading.Thread):
    def __init__(self, isUDP = False):
        ...
        self.AccX = np.zeros(0)
        self.AccY = np.zeros(0)  
        self.AccZ = np.zeros(0)
...
#                values = string.split(line, ",")
    ...
    def addFULL(self, full_values):
        ...
        self.AccX = np.append(self.GyroX,full_values[1])
        self.AccY = np.append(self.GyroY,full_values[2])
        self.AccZ = np.append(self.GyroZ,full_values[3])
        ...
        self.JinsRawData = [self.Count,self.AccX,self.AccY,self.AccZ,self.GyroX,self.GyroY,self.GyroZ,self.EogH,self.EogV]
